...
query('Q15', 'Give me all records that have both a link to an original scan and one to a KB news article, that have an associated ship whose shiptype is a subtype of kustvaarders','prefix dss: <http://purl.org/collections/nl/dss/>\n prefix mdb: <http://purl.org/collections/nl/dss/mdb/>\n prefix aat: <http://e-culture.multimedian.nl/ns/rkd/aatned/>\n \n SELECT * WHERE {\n ?record dss:hasOriginalScan ?scan.\n ?record dss:has_kb_link ?kblink.\n ?record mdb:schip ?schip.\n ?schip mdb:scheepstype ?shiptype.\n ?shiptype skos:exactMatch ?em.\n ?em skos:broader* aat:kustvaarders.\n }\n LIMIT 50').

query('Q16','Give me all records that have a link to an original scan where one person with last name "Hiemstra" enrolled.','prefix dss: <http://purl.org/collections/nl/dss/>\n prefix mdb: <http://purl.org/collections/nl/dss/mdb/>\n \n SELECT * WHERE {\n ?record dss:hasOriginalScan ?scan.\n ?record mdb:schip ?schip.\n ?pc mdb:has_aanmonstering ?record.\n ?pc mdb:persoon ?persoon.\n ?persoon mdb:achternaam ?name.\n FILTER (regex(?name, "Hiemstra"))\n \n }\n LIMIT 50').

query('Q17','Give me all records that have a link to an original scan where one person with last name "Hiemstra" enrolled and the ligplaats is matched to a geonames concept (and their coordinates)','prefix dss: <http://purl.org/collections/nl/dss/>\n prefix mdb: <http://purl.org/collections/nl/dss/mdb/>\n prefix wgs84: <http://www.w3.org/2003/01/geo/wgs84_pos#>\n \n SELECT * WHERE {\n ?record dss:hasOriginalScan ?scan.\n ?record mdb:schip ?schip.\n ?record mdb:ligplaats ?ligplaats.\n ?ligplaats skos:exactMatch ?geoligplaats.\n ?geoligplaats wgs84:long ?long.\n ?geoligplaats wgs84:lat ?lat.\n ?pc mdb:has_aanmonstering ?record.\n ?pc mdb:persoon ?persoon.\n ?persoon mdb:achternaam ?name.\n FILTER (regex(?name, "Hiemstra"))\n \n }\n LIMIT 50').


query('Q18',' Elbing: give me all voyages and their destination that had incoming flows of plant material (uses AAT link)','PREFIX elb: <http://purl.org/collections/nl/dss/elbing/> \n PREFIX aatned: <http://e-culture.multimedian.nl/ns/rkd/aatned/>  \n SELECT  ?dest ?product WHERE \n { ?voyage elb:hasDestination ?dest. \n ?voyage elb:hasProductflow ?prodflow. \n ?prodflow elb:hasIncoming ?incoming. \n ?incoming elb:hasPart ?part. \n ?part elb:hasProduct ?product .  \n ?product skos:exactMatch ?aatprod. \n ?aatprod skos:broader* aatned:plantaardig_materiaal . }
').
